...
# from training.utils import load_classifier
import training.discriminator as dis_lib

#----------------------------------------------------------------------------
# Proposed EDM sampler (Algorithm 2).

def edm_sampler_dg(
    net, latents,discriminator=None, class_labels=None,dg_weight_1st_order=0.,
    dg_weight_2nd_order=0.,time_min=0.,time_max=0., randn_like=torch.randn_like,
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        discriminator_guidance, log_ratio = dis_lib.get_grad_log_ratio(discriminator, x_hat, t_hat, net.img_resolution, time_min, time_max, class_labels)
            # if boosting:
        ...
        if i < num_steps - 1:
            ...
            if dg_weight_2nd_order != 0.:
                discriminator_guidance,log_ratio = dis_lib.get_grad_log_ratio(discriminator, x_next, t_next, net.img_resolution, time_min, time_max, class_labels)
...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, pretrained_classifier_ckpt, discriminator_ckpt,dg_weight_1st_order, dg_weight_2nd_order,cond,time_min,time_max, device=torch.device('cuda'), **sampler_kwargs):
    """Generate random images using the techniques described in the paper
    "Elucidating the Design Space of Diffusion-Based Generative Models".
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        sampler_fn = ablation_sampler if have_ablation_kwargs else edm_sampler_dg
        images = sampler_fn(net, latents,discriminator, class_labels,dg_weight_1st_order, dg_weight_2nd_order,
                            time_min,time_max, randn_like=rnd.randn_like, **sampler_kwargs)
